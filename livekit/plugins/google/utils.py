...
class _GeminiJsonSchema:
    ...
    def _simplify(self, schema: dict[str, Any], refs_stack: tuple[str, ...]) -> None:
        ...
        # NOTE: intentionally keep "default". Gemini's Schema type supports a `default`
        # field, and the realtime/Live path is forced through simplify() (Gemini Live
        # rejects parameters_json_schema, see googleapis/python-genai#1147). Dropping
        # "default" here strips pre-filled MCP params (e.g. automationId/version), so the
        # model omits them and the MCP server rejects the call with an invalid-args error.
